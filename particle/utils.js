var utils = {
    ...
    /**
     * [碰撞檢測: 矩形和矩形是否相交]
     * @param  {rect Object} r0 [矩形0]
     * @param  {rect Object} r1 [矩形1]
    ...
    quadraticBezier: function(p0, p1, p2, t, pFinal) {
        pFinal = pFinal || {};
        pFinal.x = Math.pow(1 - t, 2) * p0.x + 2 * t * (1 - t) * p1.x + t * t * p2.x;
        pFinal.y = Math.pow(1 - t, 2) * p0.y + 2 * t * (1 - t) * p1.y + t * t * p2.y;
        return pFinal;
    },
    ...
    multicurve: function(points, context) {
        ...
        //將最後兩點連起來
        context.quadraticCurveTo(points[pLen - 2].x, points[pLen - 2].y, points[pLen - 1].x, points[pLen - 1].y);
